...
to get the ABI for the `transfer` function of `moveup_account.move`, go to
the directory `moveup-move/framework/moveup-framework`.

Compile the Move packages with the Moveup CLI:
```
moveup move compile --included-artifacts all
```
...
For example:
```
cat build/MoveupFramework/abis/moveup_account/transfer.abi | od -v -t x1 -A n | tr -d ' \n'
```
*/
