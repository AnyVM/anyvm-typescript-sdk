...
import * as TokenTypes from "../moveup_types/token_types";
...
import { Token, TokenData } from "../moveup_types/token_types";
...
export class TokenClient {
   ...
   *
   * @param account MoveupAccount where collection will be created
  ...
  async createCollection(
    ...
    const builder = new TransactionBuilderRemoteABI(this.moveupClient, { sender: account.address(), ...extraArgs });
   ...
   *
   * @param account MoveupAccount where token will be created
   ...
   *
   * @param account MoveupAccount where token from which tokens will be transfered
   * @param receiver  Hex-encoded 20 byte Moveup account address to which tokens will be transfered
   * @param creator Hex-encoded 20 byte Moveup account address to which created tokens
   ...
   *
   * @param account MoveupAccount which will claim token
   * @param sender Hex-encoded 20 byte Moveup account address which holds a token
   * @param creator Hex-encoded 20 byte Moveup account address which created a token
   * @param collectionName Name of collection where token is stored
   * @param name Token name
   * @param property_version the version of token PropertyMap with a default value 0.
   * @returns The hash of the transaction submitted to the API
   ...
   *
   * @param account MoveupAccount which will remove token from pending list
   * @param receiver Hex-encoded 20 byte Moveup account address which had to claim token
   * @param creator Hex-encoded 20 byte Moveup account address which created a token
   ...
   *
   * @param sender MoveupAccount where token from which tokens will be transfered
   * @param receiver Hex-encoded 20 byte Moveup account address to which tokens will be transfered
   * @param creator Hex-encoded 20 byte Moveup account address to which created tokens
   * @param collectionName Name of collection where token is stored
   * @param name Token name
  ...
  async directTransferToken(
    sender: MoveupAccount,
    receiver: MoveupAccount,
    creator: MaybeHexString,
    collectionName: string,
    name: string,
    amount: AnyNumber,
    ...
    const builder = new TransactionBuilderRemoteABI(this.moveupClient, { sender: sender.address(), ...extraArgs });
    const rawTxn = await builder.build(
    ...
    const senderSignature = new TxnBuilderTypes.Secp256k1Signature(
      sender.signBuffer(TransactionBuilder.getSigningMessage(multiAgentTxn)).toUint8Array(),
    );

    const senderAuthenticator = new TxnBuilderTypes.AccountAuthenticatorSecp256k1(
      new TxnBuilderTypes.Secp256k1PublicKey(new HexString(sender.signingKey.getPublic("hex")).toUint8Array()),
      senderSignature,
    );

    const receiverSignature = new TxnBuilderTypes.Secp256k1Signature(
      receiver.signBuffer(TransactionBuilder.getSigningMessage(multiAgentTxn)).toUint8Array(),
    );

    const receiverAuthenticator = new TxnBuilderTypes.AccountAuthenticatorSecp256k1(
      new TxnBuilderTypes.Secp256k1PublicKey(new HexString(receiver.signingKey.getPublic("hex")).toUint8Array()),
      receiverSignature,
    );
   ...
   *
   * @param sender MoveupAccount where the token will be transferred
   * @param optIn boolean value indicates user want to opt-in or out of direct transfer
   * @returns The hash of the transaction submitted to the API
   */
  async optInTokenTransfer(sender: MoveupAccount, optIn: boolean, extraArgs?: OptionalTransactionArgs): Promise<string> {
    const builder = new TransactionBuilderRemoteABI(this.moveupClient, { sender: sender.address(), ...extraArgs });
    const rawTxn = await builder.build("0x3::token::opt_in_direct_transfer", [], [optIn]);
    const bcsTxn = MoveupClient.generateBCSTransaction(sender, rawTxn);
    const pendingTransaction = await this.moveupClient.submitSignedBCSTransaction(bcsTxn);
    return pendingTransaction.hash;
  }
  ...
  async burnByCreator(
    ...
    const builder = new TransactionBuilderRemoteABI(this.moveupClient, { sender: creator.address(), ...extraArgs });
    const rawTxn = await builder.build(
    ...
    const bcsTxn = MoveupClient.generateBCSTransaction(creator, rawTxn);
    const pendingTransaction = await this.moveupClient.submitSignedBCSTransaction(bcsTxn);
    return pendingTransaction.hash;
  }
  ...
  async burnByOwner(
    ...
    const builder = new TransactionBuilderRemoteABI(this.moveupClient, { sender: owner.address(), ...extraArgs });
    const rawTxn = await builder.build(
      "0x3::token::burn",
      [],
   ...
   *
   * @param account MoveupAccount who modifies the token properties
   ...
   * @param creator Hex-encoded 20 byte Moveup account address which created a collection
  ...
  async getCollectionData(creator: MaybeHexString, collectionName: string): Promise<any> {
    const resources = await this.moveupClient.getAccountResources(creator);
    ...
    const collectionTable = await this.moveupClient.getTableItem(handle, getCollectionTableItemRequest);
   ...
   *
   * @param creator Hex-encoded 32 byte Moveup account address which created a token
  ...
  async getTokenData(
    ...
    const collection: { type: Gen.MoveStructTag; data: any } = await this.moveupClient.getAccountResource(
      creatorHex,
    ...
    // implicit cast.
    const rawTokenData = await this.moveupClient.getTableItem(handle, getTokenTableItemRequest);
   ...
   * @param account Hex-encoded 20 byte Moveup account address which created a token
  ...
  async getTokenForAccount(account: MaybeHexString, tokenId: TokenTypes.TokenId): Promise<TokenTypes.Token> {
    const tokenStore: { type: Gen.MoveStructTag; data: any } = await this.moveupClient.getAccountResource(
    ...
    try {
      const rawToken = await this.moveupClient.getTableItem(handle, getTokenTableItemRequest);
