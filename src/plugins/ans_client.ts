...
const namePattern = new RegExp(
  "^" +
    // Optional subdomain (cannot be followed by .eth)
    "(?:(?<subdomain>[^.]+)\\.(?!eth$))?" +
    // Domain
    "(?<domain>[^.]+)" +
    // Optional .eth suffix
...
export class AnsClient {
  ...
  /**
   * Mint a new Moveup name
   *
   * @param account MoveupAccount where collection will be created
   * @param domainName Moveup domain name to mint
