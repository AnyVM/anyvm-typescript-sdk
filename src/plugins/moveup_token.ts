...
/**
 * Class for managing moveup_token
 */
export class MoveupToken {
  readonly provider: Provider;

  private readonly tokenType: string = "0x4::token::Token";

  /**
   * Creates new MoveupToken instance
   *
  ...
  private async submitTransaction(
    ...
    const rawTxn = await builder.build(`0x4::moveup_token::${funcName}`, typeArgs, args);
   ...
   *
   * @param creator MoveupAccount where collection will be created
   ...
   *
   * @param account MoveupAccount that mints the token
   * @param collection Name of collection, that token belongs to
   * @param description Token description
   * @param name Token name
   * @param uri URL to additional info about token
   * @param recipient MoveupAccount where token will be created
   * @param propertyKeys the property keys for storing on-chain properties
   * @param propertyTypes the type of property values
   * @param propertyValues the property values to be stored on-chain
   * @returns The hash of the transaction submitted to the API
