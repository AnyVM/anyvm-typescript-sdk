...
test(
  ...
  async () => {
    ...
    [account1, new Secp256k1PublicKey(account1.pubKey().toUint8Array())].forEach(async (accountOrAddress) => {
      ...
      const account2ETH = transactionRes.changes.filter((change) => {
...
test(
  ...
  async () => {
    ...
    const bcsTxn = MoveupClient.generateBCSSimulation(account1, rawTxn);
...
test(
  ...
  async () => {
    ...
    await ensureTxnSuccess(
      tokenClient.createCollection(alice, collectionName, "Alice's simple collection", "https://moveup.dev"),
    );
...
test(
  ...
  async () => {
    ...
    const pendingTxn = await client.rotateAuthKeySecp256k1(alice, new HexString(helperAccount.signingKey.getPrivate("hex")).toUint8Array());
