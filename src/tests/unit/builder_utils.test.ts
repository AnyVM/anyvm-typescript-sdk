...
describe("BuilderUtils", () => {
  ...
  it("parses a sturct TypeTag", async () => {
    ...
    const moveupCoin = new TypeTagParser(
      "0x1::coin::CoinStore < 0x1::test_coin::ETH1 ,  0x1::test_coin::ETH2 > ",
    ).parseTypeTag();
    expect(moveupCoin instanceof TypeTagStruct).toBeTruthy();
    assertStruct(moveupCoin as TypeTagStruct, "0x1", "coin", "CoinStore");

    const moveupCoinTrailingComma = new TypeTagParser(
      "0x1::coin::CoinStore < 0x1::test_coin::ETH1 ,  0x1::test_coin::ETH2, > ",
    ).parseTypeTag();
    expect(moveupCoinTrailingComma instanceof TypeTagStruct).toBeTruthy();
    assertStruct(moveupCoinTrailingComma as TypeTagStruct, "0x1", "coin", "CoinStore");

    const structTypeTags = (moveupCoin as TypeTagStruct).value.type_args;
    expect(structTypeTags.length).toBe(2);

    const structTypeTag1 = structTypeTags[0];
    assertStruct(structTypeTag1 as TypeTagStruct, "0x1", "test_coin", "ETH1");

    const structTypeTag2 = structTypeTags[1];
    assertStruct(structTypeTag2 as TypeTagStruct, "0x1", "test_coin", "ETH2");

    const coinComplex = new TypeTagParser(
      // eslint-disable-next-line max-len
      "0x1::coin::CoinStore < 0x2::coin::LPCoin < 0x1::test_coin::ETH1 <u8>, vector<0x1::test_coin::ETH2 > > >",
    ).parseTypeTag();
    ...
    expect(() => {
      new TypeTagParser("0x1::test_coin::CoinStore<0x1::test_coin::ETH").parseTypeTag();
    }).toThrow("Invalid type tag.");
    ...
    expect(() => {
      new TypeTagParser("0x1:test_coin::ETH").parseTypeTag();
    }).toThrow("Unrecognized token.");

    expect(() => {
      new TypeTagParser("0x!::test_coin::ETH").parseTypeTag();
    }).toThrow("Unrecognized token.");
    ...
    expect(() => {
      new TypeTagParser("0x1::<::CoinStore<0x1::test_coin::ETH,").parseTypeTag();
    }).toThrow("Invalid type tag.");

    expect(() => {
      new TypeTagParser("0x1::test_coin::><0x1::test_coin::ETH,").parseTypeTag();
    }).toThrow("Invalid type tag.");
