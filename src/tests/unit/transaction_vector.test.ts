 ...
 * used by the Moveup Blockchain. The test vectors are arrays of JSON objects.
...
const ENTRY_FUNCTION_VECTOR = path.join(
  ...
  "moveup_api__tests__transaction_vector_test__test_entry_function_payload.json",
);

const SCRIPT_VECTOR = path.join(
  VECTOR_FILES_ROOT_DIR,
  "moveup_api__tests__transaction_vector_test__test_script_payload.json",
);
...
function sign(rawTxn: RawTransaction, privateKey: string): string {
  ...
  const signingKey = ellipticCurve.keyFromPrivate(privateKeyBytes.slice(0, 32));
  const publicKey = new HexString(signingKey.getPublic("hex")).toUint8Array();

  const txnBuilder = new TransactionBuilderSecp256k1(
    (signingMessage) => secp256k1Sign(signingMessage, signingKey),
    //new Secp256k1Signature(new HexString(ellipticCurve.sign(signingMessage, signingKey, "hex").toDER("hex")).toUint8Array()),
    publicKey,
  );
