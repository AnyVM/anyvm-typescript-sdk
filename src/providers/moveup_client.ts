...
/**
 * Provides methods for retrieving data from Moveup node.
 */
export class MoveupClient {
  client: Gen.MoveupGeneratedClient;

  readonly nodeUrl: string;

  /**
   * Build a client configured to connect to an Moveup node at the given URL.
   *
   ...
   *
   * @param nodeUrl URL of the Moveup Node API endpoint.
  ...
  /**
   * Queries an Moveup account by address
   * @param accountAddress Hex-encoded 32 byte Moveup account address
   ...
   * @param accountAddress Hex-encoded 20 byte Moveup account address
  ...
  static generateBCSTransaction(accountFrom: MoveupAccount, rawTxn: TxnBuilderTypes.RawTransaction): Uint8Array {
    const txnBuilder = new TransactionBuilderSecp256k1((signingMessage: TxnBuilderTypes.SigningMessage) => {
      // @ts-ignore
      const sigHexStr = accountFrom.signBuffer(signingMessage);
      return new TxnBuilderTypes.Secp256k1Signature(sigHexStr.toUint8Array());
   ...
   */
  static generateBCSSimulation(accountFrom: MoveupAccount, rawTxn: TxnBuilderTypes.RawTransaction): Uint8Array {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const txnBuilder = new TransactionBuilderSecp256k1((_signingMessage: TxnBuilderTypes.SigningMessage) => {
      // @ts-ignore
      const invalidSigBytes = new Uint8Array(64);
      return new TxnBuilderTypes.Secp256k1Signature(invalidSigBytes);
    }, accountFrom.pubKey().toUint8Array());
   ...
   * @param sender Hex-encoded 20 byte Moveup account address of transaction sender
   ...
   * @param accountFrom MoveupAccount of transaction sender
  ...
  async signTransaction(
    ...
    return Promise.resolve(MoveupClient.generateBCSTransaction(accountFrom, rawTransaction));
  }
   ...
   * that event type.
   * @param address Hex-encoded 32 byte Moveup account, with or without a `0x` prefix,
   ...
   * (e.g. `0x1::coin::CoinStore<0x1::eth::ETH>`)
   ...
   *
   * @param accountOrPubkey The sender or sender's public key. When private key is available, `MoveupAccount` instance
  ...
  async simulateTransaction(
    accountOrPubkey: MoveupAccount | Secp256k1PublicKey | MultiSecp256k1PublicKey,
    rawTransaction: TxnBuilderTypes.RawTransaction,
    query?: {
    ...
    if (accountOrPubkey instanceof MoveupAccount) {
      signedTxn = MoveupClient.generateBCSSimulation(accountOrPubkey, rawTransaction);
    } else if (accountOrPubkey instanceof MultiSecp256k1PublicKey) {
      const txnBuilder = new TransactionBuilderMultiSecp256k1(() => {
        const { threshold } = accountOrPubkey;
        const bits: Seq<number> = [];
        const signatures: TxnBuilderTypes.Secp256k1Signature[] = [];
        for (let i = 0; i < threshold; i += 1) {
          bits.push(i);
          signatures.push(new TxnBuilderTypes.Secp256k1Signature(new Uint8Array(64)));
        }
        const bitmap = TxnBuilderTypes.MultiSecp256k1Signature.createBitmap(bits);
        return new TxnBuilderTypes.MultiSecp256k1Signature(signatures, bitmap);
      }, accountOrPubkey);
  ...
  async submitSignedBCSTransaction(signedTxn: Uint8Array): Promise<Gen.PendingTransaction> {
    ...
    return this.client.request.request<Gen.PendingTransaction>({
      ...
      method: "POST",
      body: signedTxn,
      mediaType: "application/x.moveup.signed_transaction+bcs",
    });
  }
   ...
   *
   * 1. Create hash message bytes: "Moveup::Transaction" bytes + BCS bytes of Transaction.
   ...
   * const bcsTxn = MoveupClient.generateBCSTransaction(sender, rawTransaction);
   * const pendingTransaction = await this.submitSignedBCSTransaction(bcsTxn);
   * const transasction = await this.moveupClient.waitForTransactionWithResult(pendingTransaction.hash);
   * ```
   */
   ...
   *
   * @param sender MoveupAccount of transaction sender.
   ...
   * `moveup move compile --save-metadata [ --included-artifacts=<...> ]`.
   * @param sender
  ...
  async estimateMaxGasAmount(forAccount: MaybeHexString): Promise<Uint64> {
    // Only Moveup utility coin is accepted as gas
    const typeTag = `0x1::coin::CoinStore<${MOVEUP_COIN}>`;
   ...
   * the account.
   * WARNING: You must create a new instance of MoveupAccount after using this function.
   * @param forAccount Account of which the auth key will be rotated
   * @param toPrivateKeyBytes New private key
   * @param extraArgs Extra args for building the transaction payload.
   * @returns PendingTransaction
   */
  async rotateAuthKeySecp256k1(
    forAccount: MoveupAccount,
    ...
    const helperAccount = new MoveupAccount(toPrivateKeyBytes);
    ...
    const challenge = new TxnBuilderTypes.RotationProofChallenge(
      ...
      new TxnBuilderTypes.AccountAddress(new HexString(authKey.slice(-40)).toUint8Array()),
    ...
    const bcsTxn = MoveupClient.generateBCSTransaction(forAccount, rawTransaction);
